...
def evaluate_model(args):
    ...
    gait_record = np.empty((args.num_iters, 6, args.num_latent_action_per_iteration*args.num_timestep_per_footstep))
            ...
            for step in range(1, args.num_timestep_per_footstep+1):
                ...
                for q in range(6):
                    if state['foot_pos'][q][2]<=-0.005:
                        gait_record[iter][q][j*args.num_timestep_per_footstep + step-1] = 1
                    else:
                        gait_record[iter][q][j*args.num_timestep_per_footstep + step-1] = 0
